...
class SimplifiedBaggingRegressor:
    ...
    def predict(self, data):
        ...
        predicts_list = []
        for bag in range(self.num_bags):
            predicts_list.append(self.models_list[bag].predict(data))
        return np.array(predicts_list).mean(axis=0)
    ...
    def _get_averaged_oob_predictions(self):
        ...
        # Your code here
        self.oob_predictions = [np.mean(pred_list) if len(pred_list) > 0 else None for pred_list in self.list_of_predictions_lists]
    ...
    def OOB_score(self):
        ...
        # Your code here
        return np.mean(
            [(zip_pred_tar[0] - zip_pred_tar[1]) ** 2 for zip_pred_tar in zip(self.oob_predictions, self.target) 
             if zip_pred_tar[0] is not None
            ]
        )
